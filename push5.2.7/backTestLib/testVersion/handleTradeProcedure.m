function [tradeTime,Marketposition,StaticEquity,DynamicEquity,LongMargin,ShortMargin] = handleTradeProcedure(bardata,traderecord,TradingUnits,MarginRatio,TradingCost_info)
%UNTITLED Summary of this function goes here
%   Detailed explanation goes here
%此函数模拟交易过程计算动态权益等

fprintf('\n%s\n','handling the trade procedure...');
...
Cash=repmat(1e6,barLength,1);               %可用资金,初始资金为100W
    ...
    while exitpos<=recordRows && tradeTime(i,:)==exitTime(exitpos,:)    %用while处理同一时间以不同价格平仓,此处无需考虑有平仓记录而此时myOpenIntRecord已经为空，
        ...
        while k <= openIntLength
            if myOpenIntRecord(k,2) == exitNum   %平仓数等于第一条建仓数
                ...
                my_profit = my_profit + compProfit(Type(myOpenIntRecord(k,1)),exitprice(exitpos),exit_OpenPosPrice,TradingUnits,exitNum,TradingCost_info);
                myOpenIntRecord(k,:) = [];
                k = k -1;
                openIntLength = openIntLength - 1;
                exitpos = exitpos+1;
            ...
            elseif myOpenIntRecord(k,2) > exitNum     %第一条建仓记录建仓数大于平仓数
                ...
                exit_OpenPosPrice = entryprice(myOpenIntRecord(k,1));
                my_profit = my_profit + compProfit(Type(myOpenIntRecord(k,1)),exitprice(exitpos),exit_OpenPosPrice,TradingUnits,exitNum,TradingCost_info); %算收益的开仓价格是平仓对应的开仓记录，但算权益的开仓价格是现有持仓的所有开仓价格
                exitpos = exitpos+1;
                break;   %平仓完成跳出循环
...
fprintf('\n%s\n','ending handling the trade procedure...');
end
