function [Type,Lots,NetMargin,RateOfReturn,CostSeries] = handleTradeRecord(traderecord,TradingUnits,TradingCost_info)
%UNTITLED3 Summary of this function goes here
%   Detailed explanation goes here

% 生成对应策略和商品的交易记录，然后根据加交易记录
% 计算收益率，胜率等等
% fprintf('\n%s\n','test version handling trade record...');
...
for i=1:recordRows
    ...
    %交易成本(建仓+平仓)
    CostSeries(i)= compTradingCost(exitprice(i),entryprice(i),TradingUnits,Lots(i),TradingCost_info);
    %CostSeries(i) = 10;
    if Type(i) == 1
...
% fprintf('\n%s\n','test version ending handling trade record...');
