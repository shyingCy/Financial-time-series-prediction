function [pareto,totalobj] = moead( mop, varargin)
...
%Set up the initial setting for the MOEA/D.
    ...
    %计算目标值排序，获取最小目标
    [V, INDS , TOTALOBJ] = arrayfun(@evaluate, repmat(mop, size(inds)), inds, 'UniformOutput', 0);
    v = cell2mat(V);
   
    idealp = min(idealp, min(v,[],2));
    
    %indcells = mat2cell(INDS, 1, ones(1,params.popsize));
    %所有个体的集合相当于CurrentPopulation
    [subproblems.curpoint] = INDS{:};
    [subproblems.totalobj] = TOTALOBJ{:}; %存储算出的所有六个目标
...
function [subproblems] = evolve(subproblems, mop, params)
    ...
    for i=1:length(subproblems)
        ...
        [obj,ind,totalobj] = evaluate(mop, ind);
        %update the idealpoint.
        idealpoint = min(idealpoint, obj);
        
        %update the neighbours.
        neighbourindex = subproblems(i).neighbour;
        subproblems(neighbourindex)=update(subproblems(neighbourindex),ind,totalobj, idealpoint);
...
function subp =update(subp, ind, totalobj ,idealpoint)
    global params
