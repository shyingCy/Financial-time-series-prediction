...
%----------策略参数，这一块需根据需要进行修改------------%
strategy = 'isakas';
Length1_Length = input('请输入Length1的参数范围，格式为(1:1:3),1为的一个值，中间的1为步长，第三个值为终止值，请输入：\n');
filter_Length = input('请输入filter的参数范围，格式为(1:1:3),1为的一个值，中间的1为步长，第三个值为终止值，请输入：\n');
                        ...
                        for filter=filter_Length
                            for Length=Length_Length
                                ...
                                if opt_temp==0 || obj{end}{opt_Way} > opt_temp
                                    ...
                                    temp_P_I(1:end) = [Length_1;filter;Length]; %这里需根据参数进行修改
                                end
                            end
                        end
                    end
