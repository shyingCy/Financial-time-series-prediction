...
%----------策略参数，这一块需根据需要进行修改------------%
strategy = 'ASCTrend';
risk_Length = input('请输入risk的参数范围，格式为(1:1:3),1为的一个值，中间的1为步长，第三个值为终止值，请输入：\n');
TrailingStart_Length = input('请输入TrailingStart的参数范围，格式为(1:1:3),1为的一个值，中间的1为步长，第三个值为终止值，请输入：\n');
                            ...
                            for TrailingStop=TrailingStop_Length
                                for StopLossSet=StopLossSet_Length
                                    ...
                                    if opt_temp==0 || obj{end}{opt_Way} > opt_temp
                                        ...
                                        temp_P_I(1:end) = [risk;TrailingStart;TrailingStop;StopLossSet]; %这里需根据参数进行修改
                                    end
                                end
                            end
                        end
