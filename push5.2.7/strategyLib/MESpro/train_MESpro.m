function [entryRecord,exitRecord,my_currentcontracts] = train_MESpro(data,pro_information,M,N,E,StopLossRate,ConOpenTimes)
...
%开仓点可作讨论
for i = 1:barLength
    if (0.4667-Time(i))<0.0001 %15.4.23 低级错误should be remember
        Begin = i+1;
        break;
    end
end
...
%         GG = GG + 1;
    ...
    %上午收盘时进行平仓
    if (abs(0.4785-Time(i)))<0.0001 %11:29:00 %15.4.23 另一个错误 已解决
        if MarketPosition == 1;
            exitPrice = Close(i);
            [exitRecord,my_currentcontracts] = train_sell(exitRecord,my_currentcontracts,...
