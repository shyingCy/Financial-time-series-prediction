function [entryRecord,exitRecord,my_currentcontracts] = train_MESproM5(data,pro_information,M,N,E,StopLossRate,ConOpenTimes)
...
%M5数据为 11:10:00
for i = 1:barLength
    if (0.4653-Time(i))<0.0001 %15.4.23 低级错误should be remember
        Begin = i+1;
        break;
    end
end
...
%         GG = GG + 1;
    ...
    %M1数据为11:29:00
    %M5数据位11:25:00
    if (abs(0.4757-Time(i)))<0.0001 %11:25:00
        if MarketPosition == 1;
            exitPrice = Close(i);
            [exitRecord,my_currentcontracts] = train_sell(exitRecord,my_currentcontracts,...
...
%         GG = GG + 1;
    ...
    if  ((((abs(0.6215-Time(i))<0.0001))&&(strcmp(pro_information(1),'IF')~=1))||((abs(0.6319-Time(i)))<0.0001))
        if MarketPosition == 1;
            exitPrice = Close(i);
            [exitRecord,my_currentcontracts] = train_sell(exitRecord,my_currentcontracts,...
                Date(i),Time(i),exitPrice,lots);
