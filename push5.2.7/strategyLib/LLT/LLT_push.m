function [ output_args ] = LLT_push( input_args )
...
q_Length = input('请输入q的参数范围，格式为(1:1:3),1为的一个值，中间的1为步长，第三个值为终止值，请输入：\n');
%TrailingStop_Length = input('请输入TrailingStop的参数范围，格式为(1:1:3),1为的一个值，中间的1为步长，第三个值为终止值，请输入：\n');
%StopLossSet_Length = input('请输入StopLossSet的参数范围，格式为(1:1:3),1为的一个值，中间的1为步长，第三个值为终止值，请输入：\n');
arg_number = 2; %参数数量
        ...
        for trainDay=trainDay_Length
            ...
            for testDay=testDay_Length
                ...
                for k=trainDay+random_up:testDay:testDayBegLength  %以testDay为步长进行测试、
                    ...
                    for Length=Length_Length
                        ...
                        for q=q_Length
                            ...
                            [profitRet,CumNetRetStd,maxDD,LotsWinTotalDLotsTotal,AvgWinLossRet,traderecord,Dy] = ...
                                train_LLT(trainData,pro_information,trainDiff,q,ConOpenTimes,isMoveOn);
                            %判断最优解
                            %存储目标temp_P和对应的参数
                            ...
                            if opt_temp==0 || obj{end}{opt_Way} > opt_temp
                                ...
                                temp_P_I(1:end) = [Length;q];%这里需根据参数进行修改
                            end
                        end
                    end
                    %-------------------------------%
                    ...
                    %==========用最优参数运行策略==========%
                    %==========这里需根据需要进行修改==========%
                    bestLength = temp_P_I(1); bestq = temp_P_I(2);
                    fprintf('%s%d%s\n','正在测试第',times,'次');
                    fprintf('训练天数为%d，测试天数为%d，最优参数为：%f %f \n',trainDay,testDay,bestLength,bestq);
                    a=2/(bestLength+1);% α值
                    for t=1:2
                        LLTvalue(t) = Close(t);
                    end
                    ...
                    d=diff(LLTvalue); %求差分
                    testDiff = d(testBeg:testEnd);
                    LLT(strategy,testData,pro_information,testDiff,bestq,ConOpenTimes);
                end
