function [entryRecord,exitRecord,my_currentcontracts] = train_Z017(data,pro_information,smallswing,largeswing,TrailingStart,TrailingStop,StopLossSet,ConOpenTimes)
...
for i=1:barLength
    ...
    if MarketPosition~=1 && Open(i) > largeswing(i) && High(i) >= smallswing(i) && smallswing(i)~=largeswing(i)
        [entryRecord,exitRecord,my_currentcontracts,isSucess] = train_buy(entryRecord,exitRecord,my_currentcontracts,...
            Date(i),Time(i),max(Open(i),smallswing(i)),1,ConOpenTimes); %这里只需修改max(Open(i),smallswing(i))，这个是价格
        %isSucess是开仓是否成功的标志
        if isSucess == 1
            BarsSinceEntry = 0; %无需止损可删除
            MyEntryPrice(1) = max(Open(i),smallswing(i)); %无需止损可删除
    ...
    if MarketPosition~=-1 && Open(i) < largeswing(i) && Low(i) <= smallswing(i) && smallswing(i)~=largeswing(i)
        [entryRecord,exitRecord,my_currentcontracts,isSucess] = train_sellshort(entryRecord,exitRecord,my_currentcontracts,...
            Date(i),Time(i),min(Open(i),smallswing(i)),1,ConOpenTimes);
        if isSucess == 1
            BarsSinceEntry = 0;
    ...
    %=====================================%
    if BarsSinceEntry == 0
        if length(MyEntryPrice) > 1
            AvgEntryPrice = mean(MyEntryPrice);
        else
            AvgEntryPrice = MyEntryPrice;
        end
