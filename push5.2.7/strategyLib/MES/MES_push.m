        ...
        for trainDay=trainDay_Length
            ...
            for testDay=testDay_Length
                ...
                for k=trainDay+up:testDay:testDayBegLength  %以testDay为步长进行测试、
                        ...
                        for E=E_Length
                            for StopLossRate=StopLossRate_Length
                                [profitRet,CumNetRetStd,maxDD,LotsWinTotalDLotsTotal,AvgWinLossRet,traderecord,Dy] = ...
                                    train_MES(trainData,pro_information,M,E,StopLossRate,ConOpenTimes,isMinPointOn)
                                %判断最优解
                                %存储目标temp_P和对应的参数
                                ...
                                if opt_temp==0 || obj{end}{opt_Way} > opt_temp
                                    ...
                                    temp_P_I(1:end) = [M;E;StopLossRate]; %这里需根据参数进行修改
                                end
                            end
                        end
                    end
                    ...
                    %==========这里需根据需要进行修改==========%
                    fprintf('%s%d%s\n','正在测试第',times,'次');
                    bestM = M; bestE = temp_P_I(2); bestStopLossRate = temp_P_I(3);
                    fprintf('训练天数为%d，测试天数为%d，最优参数为：%f %f %f %f\n',trainDay,testDay,bestM,bestE,bestStopLossRate);
                    MESpro(strategy,testData,pro_information,bestM,bestE,bestStopLossRate,ConOpenTimes);
                end
